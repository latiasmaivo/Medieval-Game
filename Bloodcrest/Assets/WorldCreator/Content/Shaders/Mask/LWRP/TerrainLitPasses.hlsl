...
half4 SplatmapFragment(Varyings IN) : SV_TARGET
{
    ...
    float2 pixPos = (IN.uvMainAndLM.xy - brushMin) / _BrushSize.xx;
    ...
    float maskValue = SAMPLE_TEXTURE2D(_Mask, sampler_Mask, IN.uvMainAndLM.xy).r;
      
    float finalValue = saturate(maskValue + brushValue);
    albedo = lerp(float3(1,1,1), float3(1,0,0), finalValue);
    InputData inputData;
